...
run_pipeline_DESEQ2 <- function(salmon.output, 
  ...
  path.to.save.output <- file.path(path.to.save.output, sprintf("sizeFactor_adjust_%s_%s", hk.gene, explicit.size.factor))
  dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)
  
  if (file.exists(file.path(path.to.save.output, "finished.csv")) == FALSE){
    ...
    print("Analysis results already exist, not generating new data..-")
    print(sprintf("Reading data from %s", file.path(path.to.save.output, "deseq_output.rds")))
...
preliminary_plot_from_deseq_output <- function(countdf, outputdir){
  ...
  ##### PCA plot
  #####----------------------------------------------------------------#####
  input.df <- input.deseq.output$norm.count[meta.data$sample]
  pca.object <- prcomp(t(input.df), rank. = 2, scale. = FALSE)
  ...
  pca.plot <- ggplot(pcadf, aes(x=PC1, y=PC2, color= condition, text = Row.names, label = SampleName)) +
    geom_point(size = 4) +
    theme_bw() + 
    theme(plot.title = element_text(hjust=0.5, face="bold", size = 12)) +
    geom_label_repel() + 
    geom_hline(yintercept = 0) +
    geom_vline(xintercept = 0)
  ggsave(plot = pca.plot, filename = "PCA.svg", path = outputdir, dpi = 300, width = 10, height = 10)
  ...
  all.hk.genes <- c("Rpl13a", "Tbp", "Hprt1", "Actb", "Gapdh")
  hkdf <- input.deseq.output$norm.count %>% subset(select = -c(gene_id)) %>%
  ...
  for (c in c("organ", "age", "condition")){
    p.compare.all.hk.genes <- hkdf.pivot %>% ggplot(aes_string(x = c, y = "exprs", fill = c)) + geom_boxplot() + 
